...
function buscarImagenes()
{
	...
	const apikey = '36088054-8c56d279ffaccf95f1be292f5';
	...
	fetch(url)
		...
		.then(resultado => 
		{
			totalPaginas = calcularPaginas(registrosPorPagina);
...
function mostrarImagenes(img)
{
	...
	//Iterar sobre las imagenes para mostrarlas
	img.forEach(imagen =>
	{
			...
			<div class="w-1/2 md:w-1/3 lg:w-1/4">
				<div class="bg-white">		
					...
					<div class="p-4">
						...
						<a href="${largeImageURL}" class="block w-full bg-blue-500 text-white uppercase font-bold text-center rounded mt-5 p-1" > Ver Imagen </a>	
					</div>
				</div>
			</div>
		`; 
...
function imprimirPaginador()
{
	...
	while (true)
	{
		...
		//caso contrario, generar los botones del generador
		const boton = document.createElement('a');
		boton.href = '#';
		boton.dataset.pagina = value;
		boton.textContent = value;
		boton.classList.add('siguiente', 'mx-auto', 'bg-yellow-400', 'px-4' ,  'py-1', 'mr-2', 'font-bold', 'mb-1', 'rounded');
